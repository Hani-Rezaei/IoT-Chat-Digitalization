...
int publish_bme280_data(void)
{
    ...
    if ((rc = MQTTPublish(&client, saul_topic_to_publish, &message)) < 0) {
        LOG_ERROR("Unable to publish (%d)\n", rc);
    }
    else {
        LOG_INFO("Message (%s) has been published to topic %s"
               "with QOS %d\n",
 ...
 */
char *generate_sensor_response(const char* topic, const char* request_unit) {
 ...
 */
void handle_mqtt_message(MessageData *data)
{
...
int mqtt_subscribe_topic(void)
{
    ...
    if (ret < 0) {
        LOG_ERROR("Unable to subscribe to %s (%d)\n",
               _topic_to_subscribe[topic_cnt], ret);
        disconnect_mqtt_client();
    }
    else {
        LOG_INFO("Now subscribed to %s, QOS %d\n",
...
/**
 * @brief Führt eine Verzögerung von ca. 2 Sekunden aus.
 *
 * Diese Funktion verwendet entweder das ZTIMER-Modul oder eine CPU-basierte 
 * Schleife als Fallback, um eine Verzögerung zu erzeugen.
 *
 * - Falls `MODULE_ZTIMER` aktiviert ist, wird `ztimer_sleep()` verwendet.
 * - Falls kein Timer-Treiber verfügbar ist, wird eine Schleife mit einer 
 *   angenommenen CPU-Taktfrequenz genutzt.
 *
 */
